...
type RosterStatus = 'all' | 'pending' | 'invited' | 'confirmed' | 'declined'
...
export function SpeakersPage({ initialStatus }: { initialStatus: RosterStatus }) {
  ...
  const visible = filterSpeakers(speakers, {
    search,
    status: initialStatus === 'all' ? 'ALL' : initialStatus.toUpperCase() as SpeakerStatus,
  })
  ...
  return <div className="flex flex-col gap-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">Profiles, portal status, sessions, tasks, and communications.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" onClick={() => setImportOpen(true)}><UploadIcon />Import CSV</Button>
        <Button variant="outline" disabled={selected.length === 0} onClick={() => setComposeOpen(true)}><MailIcon />Message {selected.length || ''}</Button>
    ...
    <Input aria-label="Search speakers" placeholder="Search name, email, title, or company" value={search} onChange={(event) => setSearch(event.target.value)} />
    {visible.length === 0 ? <EmptyState icon={<UsersIcon />} title="No matching speakers" description="Add a speaker or clear the current search and status filter." /> : <Frame>
...
function ImportDialog({ open, onOpenChange, orgId, eventId, existingEmails }: { open: boolean; onOpenChange: (open: boolean) => void; orgId: string; eventId: string; existingEmails: string[] }) {
  ...
  const preview = useMemo(() => {
    if (!source) return { headers: [] as string[], rows: [] as SpeakerCsvRow[], inserted: [] as SpeakerCsvRow[], skipped: [] as SpeakerCsvRow[], errors: [] as Array<{ row: number; message: string }> }
    try {
      ...
      return { headers: [] as string[], rows: [] as SpeakerCsvRow[], inserted: [] as SpeakerCsvRow[], skipped: [] as SpeakerCsvRow[], errors: [{ row: 0, message: cause instanceof Error ? cause.message : 'Could not parse CSV' }] }
    }
...
export function SpeakerDetailPage() {
  ...
  const { speaker, sessions } = useLoaderData('/org/:orgId/e/:eventId/speakers/:speakerId')
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_22rem]">
      ...
      <div className="flex flex-col gap-5">
        <Frame><FramePanel className="flex flex-col gap-3"><h2 className="text-sm font-medium">Files</h2>{speaker.uploadedFiles.length === 0 ? <p className="text-sm text-muted-foreground">No uploaded files.</p> : speaker.uploadedFiles.map((file) => <Link key={file.id} href={`/files/${file.id}`} className="text-sm"><span className="font-medium">{file.fileName}</span><span className="block text-xs text-muted-foreground">{file.mimeType} · {formatDateTimeUTC(file.createdAt)}</span></Link>)}</FramePanel></Frame>
