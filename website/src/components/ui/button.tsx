// Button ported from sigillo (@base-ui useRender + CVA variants).
// Auto-detects form pending state for submit buttons. `loadingText` swaps the
// label while loading (legacy akarso behavior); without it a spinner overlays
// the label. Use the `render` prop to render as a link or other element.
'use client'
...
export const buttonVariants = cva(
  "relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-lg border font-medium text-base outline-none transition-shadow before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 data-loading:select-none sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0",
  {
    ...
    variants: {
      ...
      variant: {
        // Spinner must re-set text color: data-loading:text-transparent hides the
        // label, and without these rules the spinner inherits transparent too.
        default:
...
export function Button({
  ...
  // Auto-detect form submission: show loading when this button's form is
  // pending and this is a submit button. The explicit loading prop takes
  ...
  // Overlay spinner keeps label width stable. text-transparent hides the label;
  // each variant restores color on [data-slot=button-loading-indicator] only.
  // loadingText skips the overlay and shows plain swap text instead.
  const defaultProps = {
    children: loading ? (
      <>
    ...
    className: cn(
      ...
      loading && !loadingText && 'data-loading:text-transparent',
    ),
